...
local FONT_PRESETS = {
    { name = "UI / Gotham Bold", flag = "FStringCustomFontFamily", val = "rbxasset://fonts/families/GothamSSm.json" },
    { name = "FPS / Code Monospace", flag = "FStringCustomFontFamily", val = "rbxasset://fonts/families/Inconsolata.json" },
    { name = "Pixel Style", flag = "FStringCustomFontFamily", val = "rbxasset://fonts/families/PressStart2P.json" },
    { name = "Script / Cursive", flag = "FStringCustomFontFamily", val = "rbxasset://fonts/families/Caveat.json" },
    { name = "Handdrawn", flag = "FStringCustomFontFamily", val = "rbxasset://fonts/families/Kalam.json" },
}
...
-- Section: Fonts
local fontsSection = sections["fonts"]

sectionLabel(fontsSection, "CUSTOM FONTS FFLAGS", PAD)

local fontsScroll = Instance.new("ScrollingFrame")
fontsScroll.Size = UDim2.new(1, -PAD * 2, 1, -30)
fontsScroll.Position = UDim2.new(0, PAD, 0, 24)
fontsScroll.BackgroundTransparency = 1
fontsScroll.BorderSizePixel = 0
fontsScroll.ScrollBarThickness = 4
fontsScroll.ScrollBarImageColor3 = C.border
fontsScroll.ZIndex = 143
fontsScroll.Parent = fontsSection

local fontsLayout = Instance.new("UIListLayout")
fontsLayout.FillDirection = Enum.FillDirection.Vertical
fontsLayout.SortOrder = Enum.SortOrder.LayoutOrder
fontsLayout.Padding = UDim.new(0, 10)
fontsLayout.Parent = fontsScroll

local fontsPad = Instance.new("UIPadding")
fontsPad.PaddingTop = UDim.new(0, 4)
fontsPad.PaddingBottom = UDim.new(0, 4)
fontsPad.Parent = fontsScroll

local fontInputCard = Instance.new("Frame")
fontInputCard.Size = UDim2.new(1, 0, 0, 110)
fontInputCard.BackgroundColor3 = C.surface
fontInputCard.BorderSizePixel = 0
fontInputCard.ZIndex = 144
fontInputCard.LayoutOrder = 0
fontInputCard.Parent = fontsScroll
rounded(fontInputCard, 10)
stroke(fontInputCard, C.border, 1, 0)

local fontInputTitle = Instance.new("TextLabel")
fontInputTitle.Size = UDim2.new(1, -20, 0, 20)
fontInputTitle.Position = UDim2.new(0, 12, 0, 8)
fontInputTitle.BackgroundTransparency = 1
fontInputTitle.Text = "Custom Font Path / Asset ID"
fontInputTitle.TextColor3 = C.white
fontInputTitle.TextXAlignment = Enum.TextXAlignment.Left
fontInputTitle.Font = Enum.Font.GothamBold
fontInputTitle.TextSize = 13
fontInputTitle.ZIndex = 145
fontInputTitle.Parent = fontInputCard

local fontInputBoxFrame = Instance.new("Frame")
fontInputBoxFrame.Size = UDim2.new(1, -24, 0, 32)
fontInputBoxFrame.Position = UDim2.new(0, 12, 0, 30)
fontInputBoxFrame.BackgroundColor3 = C.surface2
fontInputBoxFrame.BorderSizePixel = 0
fontInputBoxFrame.ZIndex = 145
fontInputBoxFrame.Parent = fontInputCard
rounded(fontInputBoxFrame, 6)
stroke(fontInputBoxFrame, C.border, 1, 0)

local fontInputBox = Instance.new("TextBox")
fontInputBox.Size = UDim2.new(1, -12, 1, 0)
fontInputBox.Position = UDim2.new(0, 6, 0, 0)
fontInputBox.BackgroundTransparency = 1
fontInputBox.PlaceholderText = "rbxasset://fonts/families/GothamSSm.json"
fontInputBox.PlaceholderColor3 = C.dim
fontInputBox.Text = ""
fontInputBox.TextColor3 = C.white
fontInputBox.TextXAlignment = Enum.TextXAlignment.Left
fontInputBox.Font = Enum.Font.Code
fontInputBox.TextSize = 12
fontInputBox.ClearTextOnFocus = false
fontInputBox.ZIndex = 146
fontInputBox.Parent = fontInputBoxFrame

local fontApplyCustomBtn = Instance.new("TextButton")
fontApplyCustomBtn.Size = UDim2.fromOffset(110, 30)
fontApplyCustomBtn.Position = UDim2.new(0, 12, 0, 68)
fontApplyCustomBtn.BackgroundColor3 = C.white
fontApplyCustomBtn.Text = "Apply Custom"
fontApplyCustomBtn.TextColor3 = C.bg
fontApplyCustomBtn.Font = Enum.Font.GothamBold
fontApplyCustomBtn.TextSize = 12
fontApplyCustomBtn.AutoButtonColor = false
fontApplyCustomBtn.BorderSizePixel = 0
fontApplyCustomBtn.ZIndex = 146
fontApplyCustomBtn.Parent = fontInputCard
rounded(fontApplyCustomBtn, 6)
btnFx(fontApplyCustomBtn, Color3.fromRGB(200, 200, 200), Color3.fromRGB(160, 160, 160))

local fontResetBtn = Instance.new("TextButton")
fontResetBtn.Size = UDim2.fromOffset(100, 30)
fontResetBtn.Position = UDim2.new(0, 130, 0, 68)
fontResetBtn.BackgroundColor3 = C.surface2
fontResetBtn.Text = "Reset Font"
fontResetBtn.TextColor3 = C.white
fontResetBtn.Font = Enum.Font.GothamMedium
fontResetBtn.TextSize = 12
fontResetBtn.AutoButtonColor = false
fontResetBtn.BorderSizePixel = 0
fontResetBtn.ZIndex = 146
fontResetBtn.Parent = fontInputCard
rounded(fontResetBtn, 6)
stroke(fontResetBtn, C.border, 1, 0)
btnFx(fontResetBtn, Color3.fromRGB(32, 32, 32))

fontApplyCustomBtn.MouseButton1Click:Connect(function()
    local val = fontInputBox.Text
    if val == "" then
        toast("Please enter a valid font path!")
        return
    end
    local current = editorBox.Text
    local flags = parseFlags(current)
    flags["FStringCustomFontFamily"] = val
    flags["FFlagEnableCustomFontFamily"] = "True"
    editorBox.Text = formatJson(flags)
    toast("Custom font added to editor!")
    switchSection("editor")
end)

fontResetBtn.MouseButton1Click:Connect(function()
    local current = editorBox.Text
    local flags = parseFlags(current)
    flags["FStringCustomFontFamily"] = nil
    flags["FFlagEnableCustomFontFamily"] = "False"
    editorBox.Text = formatJson(flags)
    toast("Font flags reset in editor")
    switchSection("editor")
end)

local fontsTotalH = 118
for fidx, fitem in ipairs(FONT_PRESETS) do
    local fcard = Instance.new("Frame")
    fcard.Size = UDim2.new(1, 0, 0, 54)
    fcard.BackgroundColor3 = C.surface
    fcard.BorderSizePixel = 0
    fcard.ZIndex = 144
    fcard.LayoutOrder = fidx
    fcard.Parent = fontsScroll
    rounded(fcard, 8)
    stroke(fcard, C.border, 1, 0)

    local fName = Instance.new("TextLabel")
    fName.Size = UDim2.new(1, -110, 0, 18)
    fName.Position = UDim2.new(0, 12, 0, 10)
    fName.BackgroundTransparency = 1
    fName.Text = fitem.name
    fName.TextColor3 = C.white
    ...
    fSub.Size = UDim2.new(1, -110, 0, 14)
    fSub.Position = UDim2.new(0, 12, 0, 28)
    fSub.BackgroundTransparency = 1
    fSub.Text = fitem.val
    fSub.TextColor3 = C.dim
    fSub.TextXAlignment = Enum.TextXAlignment.Left
    fSub.Font = Enum.Font.Code
    fSub.TextSize = 10
    fSub.TextTruncate = Enum.TextTruncate.AtEnd
    fSub.ZIndex = 145
    fSub.Parent = fcard

    local fApplyBtn = Instance.new("TextButton")
    fApplyBtn.Size = UDim2.fromOffset(80, 28)
    fApplyBtn.Position = UDim2.new(1, -90, 0.5, -14)
    fApplyBtn.BackgroundColor3 = C.surface2
    fApplyBtn.Text = "+ Apply"
    fApplyBtn.TextColor3 = C.white
    fApplyBtn.Font = Enum.Font.GothamBold
    fApplyBtn.TextSize = 12
    fApplyBtn.AutoButtonColor = false
    fApplyBtn.BorderSizePixel = 0
    fApplyBtn.ZIndex = 146
    fApplyBtn.Parent = fcard
    rounded(fApplyBtn, 6)
    stroke(fApplyBtn, C.border, 1, 0)
    btnFx(fApplyBtn, Color3.fromRGB(32, 32, 32))

    local fontData = fitem
    fApplyBtn.MouseButton1Click:Connect(function()
        local current = editorBox.Text
        local flags = parseFlags(current)
        flags[fontData.flag] = fontData.val
        flags["FFlagEnableCustomFontFamily"] = "True"
        editorBox.Text = formatJson(flags)
        toast("Font " .. fontData.name .. " applied to editor")
        switchSection("editor")
    end)

    fontsTotalH = fontsTotalH + 54 + 10
end

fontsScroll.CanvasSize = UDim2.new(0, 0, 0, fontsTotalH + 10)
...
aboutDesc.Text = "A high-performance FFlag manager designed for Roblox Mobile. Optimize performance, FPS, network latency, custom fonts, and client flags seamlessly."
